...
class Logic
{
  public:
    ...
    {
        // Очищстка векторов движения и состояния
        next_move.clear();
        next_best_state.clear();
        // Передача матрицы (доски), цвет игрока, позицию (ничего не выбрано), состояние, Альфа-отсечение
...
private:
                                ...
                                double alpha = -1)
    {
        ...
        // Создает копии класса turns, так как при выполнении программы прошлые данные будут затираться. Так же с beats
        ...
        for (auto turn : now_turns) {
            ...
            double score;
            if (now_have_beats) { // Если есть, что бить, то происходит вызывом рекурсивной функции для текущего игрока
                score = find_best_turns_rec(make_turn(mtx, turn), color, turn.x2, turn.y2, new_state, best_score);
            }
            else { // Если бить нечего, вызов от другого игрока
                score = find_best_turns_rec(make_turn(mtx, turn), 1 - color, 0, best_score);
            }
            if (score > best_score) { // Если score больше - обновление значения классов
